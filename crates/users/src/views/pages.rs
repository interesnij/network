...
pub async fn search_all_users_page(req: HttpRequest) -> Result<Json<Vec<CardUserJson>>, Error> {
    ...
    if params_some.is_ok() {
        ...
        if err.is_some() {
            ...
            let _res = block(move || User::search_users(params.q.clone(), params.limit, params.offset)).await?;
            Ok(Json(_res))
        }
    }
    else {
        ...
        if err.is_some() {
            ...
            if user_id > 0 {
                ...
                if _tuple.0 == false {
                    ...
                    let body = block(move || owner.search_friends(params.q.clone(), params.limit, params.offset)).await?;
                    Ok(Json(body))
                }
            }
            else {
        ...
        if err.is_some() {
            ...
            if user_id > 0 {
                ...
                if _tuple.0 == false {
                    ...
                    let body = block(move || owner.search_online_friends(params.q.clone(), params.limit, params.offset)).await?;
                    Ok(Json(body))
                }
            }
            else {
        ...
        if err.is_some() {
            ...
            if user_id > 0 && user_id != owner.id {
                ...
                if _tuple.0 == false {
                    ...
                    let user = get_user(user_id).expect("E");
                    let body = block(move || owner.search_common_friends_of_user(params.q.clone(), &user, params.limit, params.offset)).await?;
                    Ok(Json(body))
                }
            }
            else {
...
pub async fn search_user_followings_page(req: HttpRequest) -> Result<Json<Vec<CardUserJson>>, Error> {
    ...
    if params_some.is_ok() {
        ...
        if err.is_some() {
            ...
            let body = block(move || owner.search_followings(params.q.clone(), params.limit, params.offset)).await?;
            Ok(Json(body))
        }
    }
    else {
        ...
        if err.is_some() {
            ...
            if user_id > 0 {
                ...
                if _tuple.0 == false {
                    ...
                    let body = block(move || owner.search_followers(params.q.clone(), params.limit, params.offset)).await?;
                    Ok(Json(body))
                }
            }
            else {
