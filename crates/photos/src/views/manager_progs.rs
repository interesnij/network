...
pub fn manager_urls(config: &mut web::ServiceConfig) {
    ...
    config.route("/edit_user_notify", web::post().to(edit_user_notify));
    ...
    config.route("/edit_community_notify", web::post().to(edit_community_notify));
...
pub async fn create_claim_list (
    ...
    let (err, user_id) = get_user_owner_data(&req, state, data.token.clone(), 8).await;
    if err.is_some() {
        ...
        let item = get_photo_list(data.id.unwrap()).expect("E.");
...
pub async fn create_claim_photo (
    req: HttpRequest,
    state: web::Data<AppState>,
    data: Json<ReportParams>
) -> Result<Json<i16>, Error> {
...
pub async fn create_claim_comment (
    ...
    if err.is_some() {
        ...
        let item = get_photo_comment(data.id.unwrap()).expect("E.");
...
pub async fn close_comment (
    ...
    if err.is_some() {
        ...
        let item = get_photo_comment(data.item_id.unwrap()).expect("E.");
        let manager = get_user(user_id).expect("E.");
        if manager.is_moderator() {
            let _res = block (
                move || {
...
pub async fn edit_user_private (
    ...
    let (err, user_id) = get_user_owner_data(&req, state, data.token.clone(), 38).await;
    if err.is_some() {
        Err(Error::BadRequest(err.unwrap()))
    }
    else if user_id < 1 {
...
pub async fn edit_community_notify (
    req: HttpRequest,
    state: web::Data<AppState>,
    data: Json<CPrivateParams>
) -> Result<Json<i16>, Error> {
