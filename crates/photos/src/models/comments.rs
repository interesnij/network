...
use crate::models::{
    Photo, User, Community, PhotoList,
...
impl PhotoComment {
    pub fn get_comments_for_attach(ids: Vec<i32>) -> Vec<AttachPhotoCommentResp> {
        ...
        let mut stack: Vec<AttachPhotoCommentResp> = Vec::new();
        let _connection = establish_connection();
        let comments = photo_comments
            .filter(schema::photo_comments::id.eq_any(ids))
        ...
        for i in comments.iter() {
            ...
            stack.push (AttachPhotoCommentResp {
                owner:     u_resp,
                community: c_resp,
                list:      list_data,
                data:      data,
    ...
    pub fn search_comments (
        ...
        let mut community_exclude: Vec<i32> = Vec::new(); // запишем ids сообществ, у которых нельзя фото
    ...
    pub fn get_replies (
        ...
        limit:  Option<i64>,
        offset: Option<i64>,
    ) -> Result<Vec<PhotoComment>,Error>  {
        use crate::schema::photo_comments::dsl::photo_comments;

        let (_limit, _offset) = get_limit_offset(limit, offset, 20);
        let _connection = establish_connection();
        return Ok(photo_comments
    ...
    pub fn get_item(&self) -> Result<Photo, Error> {
        use crate::schema::photos::dsl::photos;

        let _connection = establish_connection();
        return Ok(photos
            .filter(schema::photos::id.eq(self.photo_id))
            .filter(schema::photos::types.lt(31))
    ...
    pub fn get_parent(&self) -> Result<PhotoComment, Error> {
        use crate::schema::photo_comments::dsl::photo_comments;
        ...
        return Ok(photo_comments
            ...
            .filter(schema::photo_comments::types.lt(10))
            .first::<PhotoComment>(&_connection)?);
    }
    ...
    pub fn close_item(&self) -> i16 {
        ...
        let o_2 = diesel::update(self)
            .set(schema::photo_comments::types.eq(self.types + 20))
            .execute(&_connection);
    ...
    pub fn delete_item(&self) -> i16 {
        ...
        let o_2 = diesel::update(self)
            .set(schema::photo_comments::types.eq(self.types + 10))
            .execute(&_connection);
    ...
    pub fn send_reaction (
        ...
        if reactions_of_list.iter().any(|&i| i==reaction_id) && list.is_user_see_el(user_id) && list.is_user_see_comment(user_id) {
            let vote_ok = photo_comment_reactions
                .filter(schema::photo_comment_reactions::user_id.eq(user_id))
                .filter(schema::photo_comment_reactions::photo_comment_id.eq(self.id))
                .first::<PhotoCommentReaction>(&_connection);

            // если пользователь уже реагировал на коммент
            if vote_ok.is_ok() {
                let vote = vote_ok.expect("E");

                // если пользователь уже реагировал этой реакцией на этот коммент
                ...
                // если пользователь уже реагировал другой реакцией на этот коммент
            ...
            // если пользователь не реагировал на этот коммент
            else {
