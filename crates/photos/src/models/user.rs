...
impl User {
    ...
    pub fn create_notify_model(&self) -> Result<UserPhotoNotification, Error> {
        use crate::models::NewUserPhotoNotification;

        let _connection = establish_connection();
        let _new_notify = NewUserPhotoNotification {
            user_id:         self.id,
            comment:         1,
            comment_reply:   1,
            mention:         1,
        ...
        let _notify = diesel::insert_into(schema::user_photo_notifications::table)
            .values(&_new_notify)
            .get_result::<UserPhotoNotification>(&_connection)?;

        return Ok(_notify);
    }
    pub fn find_notify_model(&self) -> Result<UserPhotoNotification, Error> {
        use crate::schema::user_photo_notifications::dsl::user_photo_notifications;

        let _connection = establish_connection();
        let notify = user_photo_notifications
            .filter(schema::user_photo_notifications::user_id.eq(self.id))
            .first(&_connection)?;
        return Ok(notify);
    }
    pub fn get_main_photo_list(&self) -> PhotoList {
        ...
        if _photo_list.is_ok() {
            return _photo_list.expect("E.");
        }
        else {
            use crate::models::{NewPhotoList, NewUserPhotoListPosition};
    ...
    pub fn create_main_photo_list(&self) -> () {
        use crate::schema::photo_lists::dsl::photo_lists;
        ...
        if _photo_list.is_err() {
            ...
            let _photo_list = diesel::insert_into(schema::photo_lists::table)
                .values(&new_list)
                .get_result::<PhotoList>(&_connection)
                .expect("Error saving photo_list.");
    ...
    pub fn edit_notify ( 
        ...
        let _update_field = match field {
            "comment" => diesel::update(&notify)
                .set(schema::user_photo_notifications::comment.eq(value))
                .execute(&_connection)
                .expect("E."),
            "comment_reply" => diesel::update(&notify)
                .set(schema::user_photo_notifications::comment_reply.eq(value))
                .execute(&_connection)
                .expect("E."),
            "mention" => diesel::update(&notify)
                .set(schema::user_photo_notifications::mention.eq(value))
                .execute(&_connection)
                .expect("E."),
            "comment_mention" => diesel::update(&notify)
                .set(schema::user_photo_notifications::comment_mention.eq(value))
                .execute(&_connection)
                .expect("E."),
            "repost" => diesel::update(&notify)
                .set(schema::user_photo_notifications::repost.eq(value))
                .execute(&_connection)
                .expect("E."),
            "reactions" => diesel::update(&notify)
                .set(schema::user_photo_notifications::reactions.eq(value))
                .execute(&_connection)
                .expect("E."),
            _ => 0,
        };
    ...
    pub fn get_photo_lists (
        ...
        let _connection = establish_connection();
        return photo_lists
            .filter(schema::photo_lists::user_id.eq(self.user_id))
            .filter(schema::photo_lists::community_id.is_null())
    ...
    pub fn search_photos (
        ...
        if (user_id > 0 && self.is_user_see_el(user_id))
            ...
            for list in self.get_photo_lists(Some(20), Some(0)).iter() {
    ...
    pub fn search_comments (
        ...
        while _count < _limit {
            ...
            let items = photo_comments
                .filter(schema::photo_comments::user_id.eq(self.id))
    ...
    pub fn plus_photos(&self, count: i32) -> bool {
        let _connection = establish_connection();
        diesel::update(self)
            .set(schema::users::photos.eq(self.photos + count))
            .execute(&_connection)
            .expect("Error.");
        return true;
    }
...
101 список может видеть фото
102 список может видеть комменты к фото
103 список может создавать фото
104 список может создавать комменты к фото
105 список может копировать списки / фото

111 список не может видеть фото
112 список не может видеть комменты к фото
113 список не может создавать фото
114 список не может создавать комменты к фото
115 список не может копировать списки / фото
